...
xgb_clf = xgb.XGBClassifier(learning_rate=0.3, max_depth=6, n_estimators=300, subsample=0.5, colsample_bytree=0.5, use_label_encoder=False)
xgb_clf = xgb_clf.fit(X_train_res, y_train_res)
...
acc = metrics.accuracy_score(y_test, y_predict)
print("Accuracy: ", acc)
print("Cross Validation Score: ", cross_val_score(xgb_clf, X, y, cv=10, scoring='accuracy').mean())
